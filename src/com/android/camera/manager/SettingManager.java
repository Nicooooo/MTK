...
//xcsw2 liyafeng 20170413 add storage location item begin
import com.android.camera.Storage;
import com.xchengtech.ProjectConfig;
//xcsw2 liyafeng 20170413 add storage location item end
...
public class SettingManager extends ViewManager implements View.OnClickListener,
    ...
    public void onSettingChanged(SettingListLayout settingList, ListPreference preference) {
        ...
        if(ProjectConfig.FEATURE_CAMERA_PROFESSIONAL_SUPPORT && preference.getKey().equals(SettingConstants.KEY_ISO)){
            ListPreference pref = mSettingController.getListPreference(SettingConstants.KEY_CAMERA_ZSD);
            if(preference.getValue().equals("auto")){
                pref.setValue("on");
            }else{
                pref.setValue("off");
            }
    ...
    private void initializeSettings() {
        ...
        if (ProjectConfig.XC_FEATURE_CAMERA_STORAGE_LOCATION) {
            ListPreference pref = mSettingController.getListPreference(SettingConstants.KEY_CAMERA_STORAGE_LOCATION);
            if (SettingConstants.KEY_CAMERA_STORAGE_LOCATION.equals(((ListPreference) pref).getKey())) {
                if (!Storage.isHaveExternalSDCard()) {
                    pref.setValue("internal");
    ...
    private ArrayList<ListPreference> getListPreferences(int[] keys, boolean addrestore) {
        ...
        for (int i = 0; i < keys.length; i++) {
            ...
            if (pref != null && pref.isVisibled() && pref.isShowInSetting()) {
                ...
                //XCSW2 GuoYanjie add for ProfessionalManager start
                } else if (SettingConstants.KEY_CAMERA_MANUAL_FOCUS.equals(key)
                        || SettingConstants.KEY_CAMERA_MANUAL_SHUTTER.equals(key)){
                    continue;
                //XCSW2 GuoYanjie add for ProfessionalManager end
                } else {
                    listItems.add(pref);
