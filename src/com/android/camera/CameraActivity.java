...
import android.view.View;
import android.view.WindowManager;
import com.mediatek.camera.ui.FrontCameraWhiteView;
...
public class CameraActivity extends ActivityBase implements
    ...
    private boolean mIsToastShowed = false;
    //add by liyafeng for KEYWLVBS-166 20170110 (end)
    //Add by liyafeng 20170225 start
    private FrontCameraWhiteView mRootView;
    private View mFrontCameraWhiteView;
    private boolean mIsFrontCameraWhiteViewAdded = false;
    //Add by liyafeng 20170225 end
    ...
    public void onCreate(Bundle icicle) {
        ...
        Storage.setSettingCtrl(mISettingCtrl);
    ...
    protected void onResume() {
        ...
        ViewGroup appRoot = (ViewGroup) findViewById(R.id.camera_app_root);
        //XCSW2 GuoYanjie modify for CameraFullScreen start
        int flag = 0;
        if (hasNavigationBar() && ProjectConfig.FEATURE_CAMERA_FULLSCREEN_SUPPORT){
        ...
        //XCSW2 GuoYanjie modify for CameraFullScreen end
        CameraPerformanceTracker.onEvent(TAG,
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if(ProjectConfig.XC_ADD_VOLUME_FUNCTION_SETTING_MENU_FOR_CAMERA){
            String volumeMode = mISettingCtrl.getSettingValue(SettingConstants.KEY_VOLUME_RESPONSE);
            switch(keyCode){
                case KeyEvent.KEYCODE_VOLUME_UP:
                case KeyEvent.KEYCODE_VOLUME_DOWN:
                    if("zoom".equals(volumeMode)){
                        Log.i(TAG,"onkeydown zoom");
    ...
    public void showGridLines() {
        boolean isGridShow = SETTING_ON.equals(mISettingCtrl.getSettingValue(
            SettingConstants.KEY_CAMERA_GRID_LINES));
        //Log.e(TAG, "HelloWorld isGridShow = " + isGridShow);
        mPreviewFrameLayout.showGrid(isGridShow);
    }
    ...
    private void notifyParametersReady() {
        ...
        //XCSW2 GuoYanjie add for GridLines start
        if (ProjectConfig.FEATURE_CAMERA_GRID_LINES_SUPPORT) {
            showGridLines();
        }
        //XCSW2 GuoYanjie add for GridLines end
    }
        ...
        public void onRestorePreferencesClicked() {
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    //XCSW2 GuoYanjie add for DefaultPictureRatio start
                    SettingUtils.restorePreviewRatioToDefault(
                            (Context) CameraActivity.this,
                            mCameraDeviceCtrl.getParametersExt(),
                            getSharePreferences(mCameraDeviceCtrl.getCameraId()),
                            mCameraDeviceCtrl.getCameraId());
                    //XCSW2 GuoYanjie add for DefaultPictureRatio end
    ...
    private PickerManager.PickerListener mPickerListener = new PickerManager.PickerListener() {
        ...
        //Add by liyafeng 20170225 start
        @Override
        public boolean onWhiteLcdPicked(String mode) {
            mISettingCtrl.onSettingChanged(SettingConstants.KEY_WHITE_LCD, mode);
            mCameraDeviceCtrl.applyParameters(false);
            return true;
        }
        //Add by liyafeng 20170225 end
    ...
    private void installIntentFilter() {
        ...
        //add by liyafeng for KEYWLVBS-166 20170110 (start)
        registerReceiver(mBatteryIntentReceiver, mBatteryIntentFilter);
        //add by liyafeng for KEYWLVBS-166 20170110 (end)
    }
    ...
    private void uninstallIntentFilter() {
        ...
        //add by liyafeng for KEYWLVBS-166 20170110 (start)
        unregisterReceiver(mBatteryIntentReceiver);
        //add by liyafeng for KEYWLVBS-166 20170110 (end)
    }
    ...
    private final BroadcastReceiver mBatteryIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                if (level >= 0 && level <= 15) {
                    if (!mIsToastShowed) {
                        Toast.makeText(context, R.string.show_flashlight_toast, Toast.LENGTH_LONG).show();
                        mISettingCtrl.onSettingChanged(SettingConstants.KEY_FLASH, Parameters.FLASH_MODE_OFF);
                        mIsToastShowed = true;
                    }
                } else {
                    mIsToastShowed = false;
    ...
    private void transparentNavigationBar() {
        ...
        window.setNavigationBarColor(0x00FFFFFF);
    }

    private boolean hasNavigationBar() {
        IWindowManager wmService = WindowManagerGlobal.getWindowManagerService();
        if (wmService != null) {
            try {
                return wmService.hasNavigationBar();
    ...
    public void addWhiteView() {
        if (!ProjectConfig.XC_FEATURE_FRONT_CAMERA_WHITE_LCD) {
            return;
        }

        if (!isWhiteLcdEnabled()) {
            return;
        }
        ...
        if (mRootView == null) {
            mRootView = new FrontCameraWhiteView(CameraActivity.this);
        }
        if (mRootView != null) {
            if (!mIsFrontCameraWhiteViewAdded) {
                mFrontCameraWhiteView = mRootView.getFrontCameraWhiteView();
                mIsFrontCameraWhiteViewAdded  = true;
                android.util.Log.d("zhangtao", "add white lcd view success");
            }
        }
        if (mFrontCameraWhiteView != null) {
            mFrontCameraWhiteView.setBackgroundColor(android.graphics.Color.WHITE);
            mFrontCameraWhiteView.setVisibility(View.VISIBLE);
        }
    }
    ...
    public void removeWhiteView() {
        ...
        if (mFrontCameraWhiteView != null) {
            mFrontCameraWhiteView.setVisibility(View.GONE);
        }
        if (mFrontCameraWhiteView != null && mIsFrontCameraWhiteViewAdded) {
            WindowManager wm = (WindowManager)CameraActivity.this.getSystemService(Context.WINDOW_SERVICE);
            wm.removeView(mFrontCameraWhiteView);
            mIsFrontCameraWhiteViewAdded = false;
            android.util.Log.d("zhangtao", "remove white lcd view success");
        }
    }

    private boolean isWhiteLcdEnabled() {
        String whiteLcd = mISettingCtrl.getSettingValue(SettingConstants.KEY_WHITE_LCD);
        android.util.Log.d("zhangtao", "whiteLcd:"+whiteLcd);
        if ("on".equals(whiteLcd)) {
            return true;
        }
        return false;
    }
