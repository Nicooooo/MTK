...
public class CameraDeviceExt implements ICameraDeviceExt {
    ...
    private boolean isFrontCamera() {
        return mCameraId == android.hardware.Camera.CameraInfo.CAMERA_FACING_FRONT;
    }
    private boolean isBackCamera() {
        return mCameraId == android.hardware.Camera.CameraInfo.CAMERA_FACING_BACK;
    }
    private boolean isInited() {
        return mPreferences.contains(SettingConstants.KEY_PICTURE_RATIO);
    }
    ...
    public void setPreviewSize() {
        ...
        if (pictureRatio == null) {
            ...
            if (supportedRatios != null && supportedRatios.size() > 0) {
                ...
                //XCSW2 GuoYanjie add for DefaultPictureRatio start
                if (ProjectConfig.FEATURE_FRONT_CAMERA_DEFAULT_RADIO_4_3 && isFrontCamera() && !isInited()) {
                    ratioString = SettingUtils.getRatioString(4d / 3);
                }
                if (ProjectConfig.FEATURE_BACK_CAMERA_DEFAULT_RADIO_4_3 && isBackCamera() && !isInited()) {
                    ratioString = SettingUtils.getRatioString(4d / 3);
                }
                //XCSW2 GuoYanjie add for DefaultPictureRatio end
