...
public class VideoMode extends CameraMode implements MediaRecorder.OnErrorListener,
    ...
    protected boolean startVideoRecording() {
        ...
        //xcsw2 huanglijun 20170105 for video button sound(start)
        boolean  soundEnabled = "on".equals(mISettingCtrl.getSettingValue(SettingConstants.KEY_CAMERA_SHUTTER_SOUND));
        if (soundEnabled) {
    ...
    protected void initializeShutterStatus() {
        ...
        //XCSW2 GuoYanjie modify for FrontCameraSupportVss start
        if ((!ProjectConfig.FEATURE_FRONT_CAMERA_VIDEO_SNAPSHOT_SUPPORT
                && CameraInfo.CAMERA_FACING_FRONT == mICameraDeviceManager
        ...
        //XCSW2 GuoYanjie modify for FrontCameraSupportVss end
            mICameraAppUi.setPhotoShutterEnabled(false);
        } else {
    ...
    private void setEisFramesToRecorder() {
        if ("on".equals(mISettingCtrl .getSettingValue(SettingConstants.KEY_VIDEO_EIS))) {
            ...
            try {
                mMediaRecorder.setParametersExtra("media-param-eis="
