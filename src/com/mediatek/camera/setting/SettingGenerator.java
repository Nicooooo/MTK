...
public class SettingGenerator {
    ...
    public void restoreSetting(int cameraId) {
        ...
        if (supportedFBMode != null && supportedFBMode.size() > 0) {
            ...
            //----------chenlong added for KEYWYPSTV-124 20170111 start --------------
            if(ProjectConfig.FEATURE_CUSTOME_FOR_POSITIVO){
                defaultFBMode = supportedFBMode.get(supportedFBMode.size()-1);
            }
            //----------chenlong added for KEYWYPSTV-124 20170111  end --------------
    ...
    private String generateDefaultValue(String key, Parameters parameters
        ...
        if (SettingConstants.KEY_MULTI_FACE_BEAUTY.equals(key)) {
            ...
            if (supportedFBMode != null && supportedFBMode.size() > 0) {
                //----------chenlong added for KEYWYPSTV-124 20170111 start --------------
                if(ProjectConfig.FEATURE_CUSTOME_FOR_POSITIVO){
                    return supportedFBMode.get(supportedFBMode.size()-1);
                }else{
    ...
    private boolean filterPreference(final ListPreference preference) {
        ...
        case SettingConstants.ROW_SETTING_AIS:
            //wangleiquan for anti-shake (start)
            if (ProjectConfig.FEATURE_CAMERA_REMOVE_ANTI_SHAKE) {
            ...
            //wangleiquan for anti-shake (end)
            break;
        ...
        case SettingConstants.ROW_SETTING_ZSD:
            //wangleiquan for zsd
            if (mICameraContext.getFeatureConfig().isLowRamOptSupport() || ProjectConfig.FEATURE_CAMERA_REMOVE_ZSD) {
                removePreference = true;
            } else {
                removePreference = filterUnsupportedOptions(preference,
                        ParametersHelper.getParametersSupportedValues(parameters, key), settingId);
        ...
        case SettingConstants.ROW_SETTING_SLOW_MOTION:
            ...
            //xcsw2 huanglijun 20160110 for KEYWLVBS-72 hide SLOW_MOTION menu (Begin)
            if (ProjectConfig.CAMERA_SETTING_SLOW_MOTION_HIDE_SUPPORT) {
                removePreference = true;
            }
            //xcsw2 huanglijun 20160110 for KEYWLVBS-72 hide SLOW_MOTION menu (End)
            break;
        ...
        case SettingConstants.ROW_SETTING_MULTI_FACE_MODE:
            //wangleiquan for face beauty (start)
            if (ProjectConfig.FEATURE_CAMERA_REMOVE_FACE_BEAUTY) {
                removePreference = true;
            } else {
                // add for vFB
            ...
            //wangleiquan for face beauty (end)
            break;
        ...
        case SettingConstants.ROW_SETTING_VOLUME_RESPONSE:
        ...
        case SettingConstants.ROW_SETTING_CAMERA_GRID_LINES:
        ...
        case SettingConstants.ROW_SETTING_CAMERA_MANUAL_FOCUS:
            removePreference = filterUnsupportedOptions(preference,
                    ParametersHelper.getParametersSupportedValues(parameters, key), settingId);
            ...
            break;
        case SettingConstants.ROW_SETTING_CAMERA_MANUAL_SHUTTER:
            removePreference = filterUnsupportedOptions(preference,
                    ParametersHelper.getParametersSupportedValues(parameters, key), settingId);
            if(ProjectConfig.FEATURE_CAMERA_PROFESSIONAL_SUPPORT){
                removePreference = false;
        ...
        case SettingConstants.ROW_SETTING_WHITE_LCD:
        ...
        case SettingConstants.ROW_SETTING_TIMESTAMP:
            if (!ProjectConfig.XC_FEATURE_CAMERA_TIMESTAMP) {
                removePreference = true;
            }
            break;
        //xcsw2 liyafeng 20170323 add timestamp item end
        //xcsw2 liyafeng 20170405 add storage location item begin
        case SettingConstants.ROW_SETTING_STORAGE_LOCATION:
            if (!ProjectConfig.XC_FEATURE_CAMERA_STORAGE_LOCATION) {
                removePreference = true;
            }
            break;
    ...
    private void limitPreferencesByIntent() {
        ...
        //Add by liyafeng 20170225 start
        if (currentCameraId == mICameraDeviceManager.getBackCameraId()) {
            ArrayList<ListPreference> preferences = mPreferencesMap.get(currentCameraId);
            int[] unSupportedByBackCamera = SettingConstants.UN_SUPPORT_BY_BACK_CAMERA;
            for (int i = 0; i < unSupportedByBackCamera.length; i++) {
                preferences.set(unSupportedByBackCamera[i], null);
            }
        }
        //Add by liyafeng 20170225 end
    }
    ...
    private boolean buildPictureSizeEntries(ListPreference pref, List<String> supportedSizes) {
        ...
        //XCSW2 wucaihong 20170331 KYJSEFLY-110 start
        String[] myCameraValues=mContext.getResources().getStringArray(R.array.pref_camera_entry_values);
        String[] myCameraEntries=mContext.getResources().getStringArray(R.array.pref_camera_entries);
        //if (ProjectConfig.FEATURE_CAMERA_PICTURE_SIZE_CUSTUMER) {
        if(ProjectConfig.FEATURE_CAMERA_PICTURE_SIZE_CUSTUMER){
            String entry="";
            int index = 0;
            for (int i = 0; i < supportedSizes.size(); i++) {
                for (int m = 0; m < myCameraValues.length; m++) {
                    if (supportedSizes.get(i).equals(myCameraValues[m])) {
                        entry = myCameraEntries[m];
        ...
        //XCSW2 wucaihong 20170331 KYJSEFLY-110 end
